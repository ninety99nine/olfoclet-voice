<template>
    ...
    <div class="bg-gray-50 pt-4 p-2 space-y-4">

      <!-- Performance Bar -->
      <div class="w-full flex items-center rounded-2xl shadow-md border border-gray-200 overflow-y-auto">
        ...
        <div class="flex flex-grow pl-8">
            ...
            <div class="flex-1 flex items-center space-x-3">
                ...
                <div>
                    <p class="text-md font-semibold text-gray-900">1245</p>
            ...
            <!-- Avg. Call Duration -->
            <div class="flex-1 flex items-center space-x-3">
            <PhoneCall class="h-6 w-6 text-indigo-600" />
            <div>
                <p class="text-md font-semibold text-gray-900">3m 0s</p>
            ...
            <!-- Avg. Wait Time -->
            <div class="flex-1 flex items-center space-x-3">
            <Timer class="h-6 w-6 text-red-600" />
            <div>
            ...
            <div>
                <p class="text-md font-semibold text-green-600">78%</p>
      ...
      <div class="flex h-[calc(100vh-60px)] space-x-2">
            ...
            <div v-for="(conversation, index) in conversations" :key="index" @click="selectConversation(index)"
              ...
              <div class="flex items-center space-x-3">
                <div class="flex-shrink-0">
                  <span class="inline-flex items-center justify-center h-6 w-6 rounded-full text-xs font-medium" :class="conversation.badgeClass">
                    <img v-if="conversation.channel === 'Call'" :src="'/images/calling.gif'" class="w-4" />
                    <span v-else>{{ conversation.channelInitial }}</span>
                  </span>
                </div>
                <div class="flex-1 relative">
                    ...
                    <div v-if="conversation.channel === 'Call' && conversation.critical" class="w-2 h-2 bg-red-500 rounded-full absolute bottom-1 right-0"></div>
                    <div v-if="conversation.channel === 'Call' && !conversation.critical" class="w-2 h-2 bg-yellow-500 rounded-full absolute bottom-1 right-0"></div>
                </div>
        ...
        <div class="w-1/2 flex flex-col">
          ...
          <div v-if="conversations[selectedConversation].channel === 'Call' && conversations[selectedConversation].callStatus === 'incoming' && !isAnswered"
            class="relative rounded-2xl shadow-xl shadow-fuchsia-100 mb-6">
  ...
  import { Bold, Redo, Italic, Link, Smile, X, MicOff, Pause, ArrowRight, PlusCircle, Tag, Moon, Phone, Check, CheckCheck, IterationCcw, ArrowRightLeft, Clock, PhoneCall, Timer, CheckCircle } from 'lucide-vue-next';
  ...
  .glow::before {
    ...
    position: absolute;
    left: 50%;
    width: 650px; /* Large enough to cover the card when rotated */
    height: 650px; /* Taller to ensure the gradient sweeps around the border */
    ...
    transform-origin: center center; /* Rotate around the card’s center */
