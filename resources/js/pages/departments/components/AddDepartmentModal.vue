<template>
    <Modal
        ...
        approveText="Create Department"
        :approveAction="createDepartment"
        :approveLoading="isCreating"
        subheader="Add a new department to the platform">
...
export default {
    ...
    methods: {
        ...
        async createDepartment(hideModal) {
            this.formState.hideFormErrors();

            if (this.form.name.trim() === '') {
                this.formState.setFormError('name', 'Department name is required');
            }
            ...
            try {
                ...
                this.notificationState.showSuccessNotification('Department created');
                this.$emit('created');
                hideModal();
            } catch (error) {
                const message = error?.response?.data?.message || error?.message || 'Something went wrong trying to create the department';
