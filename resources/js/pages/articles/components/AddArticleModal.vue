<template>
    <Modal
        ...
        approveText="Create Article"
        :approveAction="createArticle"
        :approveLoading="isCreating"
        subheader="Add a new article to a knowledge base">
    ...
    methods: {
        ...
        async createArticle(hideModal) {
            ...
            try {
                ...
                this.notificationState.showSuccessNotification('Article created');
                this.$emit('created');
                hideModal();
            } catch (error) {
                const message = error?.response?.data?.message || error?.message || 'Something went wrong trying to create the article';
